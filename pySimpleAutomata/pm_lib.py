...
def jffToNFA(arquivo):
    ...
    for k in doc['structure']['automaton']['transition']:
        transitions.setdefault(('s' + k['from'], k['read']), set()).add('s' + k['to'])
...
def min_aux_2(P, X, transitions, statesAlphabet):
    ...
    for i in range(len(X)-1):
        ...
        for j in range(len(R)):
            equal_states = True
            alphabet = set(statesAlphabet[R[j][0]]).union(set(statesAlphabet[X[i+1]]))
            for a in alphabet:
                dest1 = None
                dest2 = None
                if (R[j][0], a) in transitions: dest1 = transitions[R[j][0],a]
                if (X[i+1],a) in transitions: dest2 = transitions[X[i+1],a]
                equal_dest = True
                if(dest1!=dest2):
                    equal_dest = False
...
def rename(dfa, prefix_name ="s"):
    ...
    for s in dfa["states"]:
        if(s==dfa["initial_state"]): 
            ...
            if (s in dfa['accepting_states']): accepting_states.add(new_state)
        else: 
    ...
    for t in dfa["transitions"]:
        transitions[tStates[t[0]],t[1]] = tStates[dfa["transitions"][t[0],t[1]]]
...
def getStateTransitionAlphabet(dfa):
    ...
    for t in transitions:
        if(t[0] in statesAlphabet): statesAlphabet[t[0]].add( t[1])
        else: statesAlphabet[t[0]] = {t[1]} 
    for s in states:
        if(not s in statesAlphabet): statesAlphabet[s] ={} 
...
def minimization(dfa):
    ...
    list_non_accepting_states = list(dfa["states"].difference(dfa["accepting_states"]))#.sort() 
    ...
    for S in P:
        ...
        for a in alphabet:
            ...
            if((S[0],a) in transitions ): state_destination = transitions[(S[0],a)]
...
def exemplo1():
    ...
    automata_IO.dfa_to_dot(rename(minimization(dfa)), 'dfa_min_trim', 'exemplo1')
...
def exemplo2():
    ...
    #print(minimization(dfa))
    automata_IO.dfa_to_dot(rename(minimization(dfa)), 'dfa_min_trim', 'exemplo2')
...
def exemplo3():
    ...
    #print(minimization(dfa))
    automata_IO.dfa_to_dot(rename(minimization(dfa)), 'dfa_min_trim', 'exemplo3')
...
def toNFA(lLog):
    ...
    for j in range(len(lLog)):
        for k in range(len(lLog[j])):
            ...
            if(k==0):
                transitions.setdefault(('s0', lLog[j][k]), set()).add('s'+str(i))
            else:
                transitions.setdefault(('s'+str((i-1)), lLog[j][k]), set()).add('s'+str(i))
            i += 1
...
#        print(l)
    ...
    for j in range(len(lLog)):
        for k in range(len(lLog[j])):
            ...
            if(k==0):
                fSave.write("\t\ts0 -> s"+str(i)+ "[label=\""+ lLog[j][k]+"\"]\n")
            else:
                fSave.write("\t\ts"+str((i-1))+" -> s"+str(i)+ "[label=\""+ lLog[j][k]+"\"]\n")
            i += 1
    fSave.write("}")
...
def convertToDOTFile(file_name, file_xes):
    ...
    for case_index, case in list(enumerate(log)).sort():
        ...
        for event_index, event in enumerate(case):
            ...
            if(event_index==0):
                trace = log[case_index][event_index]["concept:name"]
                fSave.write("\t\ts0 -> s"+str(i)+ "[label=\""+ event["concept:name"]+"\"]\n")
            else:
                trace+=", "+log[case_index][event_index]["concept:name"]
                fSave.write("\t\ts"+str((i-1))+" -> s"+str(i)+ "[label=\""+ event["concept:name"]+"\"]\n")
            i += 1
